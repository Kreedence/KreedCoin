...
contract KreedCoin is IERC20Upgradeable, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    constructor() {
        _disableInitializers();
    }
    ...
    function initialize() public initializer {
        ...
        _maxWalletBalance = 100000 * 10**18;
        swapAmount = 20000 * 10**18;
        ...
        __KreedCoin_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __KreedCoin_tiers_init() internal initializer {
        _mainFees = _addTier(0, 100, 100, 0, 0, address(0), address(0));
    }
        ...
        checkTierIndex(_tierIndex)
    {
        ...
        if (_tierIndex == 0) {
            _mainFees.ecoSystemFee = _ecoSystemFee;
        }
    }
        ...
        checkTierIndex(_tierIndex)
    {
        ...
        if (_tierIndex == 0) {
            _mainFees.liquidityFee = _liquidityFee;
        }
    }
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "Address Zero is not allowed");
        excludeFromReward(_ecoSystem);
        feeTiers[_tierIndex].ecoSystem = _ecoSystem;
        if (_tierIndex == 0) {
            _mainFees.ecoSystem = _ecoSystem;
        }
    }
    ...
    function updateRouterAndPair(address _newRouter) public onlyOwner {
        IUniswapV2Router02 _newUniswapV2Router = IUniswapV2Router02(_newRouter);
    ...
    function setLiquiditySwapEnabled(bool _swapEnabled) external onlyOwner {
        liquiditySwapEnabled = _swapEnabled;
    }
    ...
    function setMaxWalletBalance(uint256 newAmount) public onlyOwner {
        _maxWalletBalance = newAmount;
    }
        ...
        isRouter(_msgSender())
    {
        ...
        if(from != owner() && to != owner() && from != address(this) && to != address(uniswapV2Pair) && to != address(_burnAddress)){
            require(balanceOf(to) + amount <= _maxWalletBalance, "Recipiant balance exceeds maxWalletBalance");
        }

        if(balanceOf(address(this)) >= swapAmount && !inLiquiditySwap && from != address(uniswapV2Pair) && liquiditySwapEnabled){
            _liquiditySwap();
        }
    ...
    function _swapTokensForBNB(uint256 _amount) private {
    ...
    function _liquiditySwap() private {
        inLiquiditySwap = true;
        uint256 half = balanceOf(address(this)) / 2;
        uint256 otherHalf = balanceOf(address(this)) - half;
        uint256 initialBalance = address(this).balance;
        _swapTokensForBNB(half);
        uint256 newBalance = address(this).balance - initialBalance;
        _addLiquidity(otherHalf, newBalance);
        inLiquiditySwap = false;
    }
    ...
    function _removeLiquidityETHSupportingFeeOnTransferTokens(uint percent) private {
        uint256 lpBalance = IERC20Upgradeable(uniswapV2Pair).balanceOf(address(this)) - MINIMUM_LIQUIDITY;
        uint256 lpAmount = lpBalance * percent / 10000;
        IERC20Upgradeable(uniswapV2Pair).approve(address(uniswapV2Router), lpAmount);
    ...
    function addLiquidity(uint256 _amount, uint256 _ethAmount) public onlyOwner {
        _addLiquidity(_amount, _ethAmount);
    }

    function removeLiquidity(uint256 percent) public onlyOwner {
        _removeLiquidityETHSupportingFeeOnTransferTokens(percent);
    }

    function setSwapAmount(uint256 _numToken) public onlyOwner {
        swapAmount = _numToken;
    }
    ...
    return IERC20Upgradeable(uniswapV2Pair).balanceOf(address(this)) - MINIMUM_LIQUIDITY;
    }
